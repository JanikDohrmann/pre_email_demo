...
use recrypt::api::EncryptedValue::{EncryptedOnceValue, TransformedValue};
use recrypt::api::*;
...
/// Function for re-encrypting WC09 encrypted messages.
/// # Arguments
/// * `target` - The re-encryption target.
/// * `message` - The message to re-encrypt.
/// * `signing_keys` - The signing keys of the delegator.
/// # Return
/// Returns a Message with the re-encrypted text. The subject and from address are identical to the incoming message.
pub fn re_encrypt(target: Target, message: Message, signing_keys: SigningKeypair) -> Message {
    println!("\nDer Ciphertext wird zuerst in das richtige Format für die Umschlüsselung konvertiert.");
    let enc_value = construct_enc_value(message.text);

    println!("\nDanach findet die Umschlüsselung des Ciphertexts statt.\nDafür wird der Ciphertext, der Re-Encryption-Schlüssel und der Signaturschlüssel benötigt.");
    let recrypt = Recrypt::new();
    let transformed_value = recrypt
        .transform(enc_value, target.re_key, &signing_keys)
        .unwrap();

    println!("\nDie Nachricht wird zurück in das Transportformat konvertiert.\nDieses hat die Form xxx xxx xxx wobei jeder Block ein Byte als Dezimalwert darstellt.");
    let text = deconstruct_encrypted_value(transformed_value);
...
fn construct_enc_once_value(text_vec: Vec<u8>) -> EncryptedValue {
...
fn construct_transformed_value(text_vec: Vec<u8>) -> EncryptedValue {
    let epk_split = text_vec.split_at(PUBLIC_KEY_SIZE);
    let epk = construct_public_key(epk_split.0).unwrap();
...
/// xxx xxx xxx ...  where x is a digit and the triplets xxx represent an byte of the encrypted value.
fn deconstruct_encrypted_once_value(encrypted_value: EncryptedValue) -> String {
    let mut vec = Vec::new();
...
/// xxx xxx xxx ...  where x is a digit and the triplets xxx represent an byte of the encrypted value.
fn deconstruct_transformed_value(transformed_value: EncryptedValue) -> String {
    let mut vec = Vec::new();
...
pub fn construct_transform_key(key: String) -> TransformKey {
    ...
    let epk_split = vec.split_at(PUBLIC_KEY_SIZE);
    let ephemeral_public_key = construct_public_key(epk_split.0).unwrap();

    let tpk_split = epk_split.1.split_at(PUBLIC_KEY_SIZE);
    ...
    let psk_split = htk_split.1.split_at(PUBLIC_SIGNING_KEY_SIZE);
...
mod tests {
    ...
    use crate::re_encryption::{construct_enc_value, re_encrypt, deconstruct_encrypted_value, deconstruct_plaintext, construct_plaintext};
    ...
    /// Test for transformation of plaintext.
    #[test]
    fn transform_once_test() {
        ...
        let enc = construct_enc_value(transformed_message.text);
