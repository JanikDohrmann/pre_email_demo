...
use crate::re_encryption::construct_transform_key;
use crate::target::{build_target, Target};
use pest::Parser;
...
mod tests {
    ...
    use crate::re_encryption::deconstruct_transform_key;
    use recrypt::api::*;

    /// Parse test with test@localhost as address
    #[test]
    fn parse_start_command_test_easy() {
        let recrypt = Recrypt::new();

        let alice_signing_keypair = recrypt.generate_ed25519_key_pair();

        let (alice_priv_key, _alice_pub_key) = recrypt.generate_key_pair().unwrap();
        let (_bob_priv_key, bob_pub_key) = recrypt.generate_key_pair().unwrap();
    ...
    /// Parse test with max.mustermann@mail.de as address
    #[test]
    fn parse_start_command_test() {
        ...
        let test_re_key = deconstruct_transform_key(alice_to_bob_transform_key.clone());
        let test_address = "max.mustermann@mail.de".to_string();
        let test_string = format!("START\nTA:{}\nRK:{}", test_address, test_re_key);
        let t = parse(test_string);
